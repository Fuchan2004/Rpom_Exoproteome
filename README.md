...
```
python scripts/fragpipe/statistics-fragpipe_multipletests.py <input_filename_1> <input_filename_2> <test_name> <multiple_test_correction>
```

Output files are saved in a new folder called `output/proteomics/` which is created upon running this code. 

#### 4) Plotting
Volcano plots are generated by running the script `scripts/fragpipe/volcano_plot_fragpipe.py`. The script contains a function to plot -log10 transformed p-values against log2 fold changes (from triplicate measurements) of two measurements. In Terminal, run:

```
for file in output/proteomics/*.txt
do
python volcano_plot_fragpipe.py '$file' figures/proteomics
done
```
