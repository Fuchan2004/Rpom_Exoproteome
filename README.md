...
```
python scripts/fragpipe/format_raw.py ./data/proteomics/raw/
```
...
#### 3) Statistics
Statistical analyses of the proteomics output are essential for gaining confidence in our data. We will use the script `scripts/fragpipe/statistics_multipletests.py` to conduct statistical analyses on our fragpipe outputs. In summary the provided scripts will:
...
#### 4) Plotting
Volcano plots are generated by running the script `scripts/fragpipe/volcano_plot_fragpipe.py`. The script contains a function to plot -log10 transformed p-values against log2 fold changes (from triplicate measurements) of two measurements. We provide a bash script `scripts/fragpipe/run_volcano_list.sh`that allows you to run volcano plots and extract statistics data directly from the files generated in `output/proteomics/`. Run it in Terminal from the root directory of this repository:

```
bash scripts/fragpipe/run_volcano_list.sh output/proteomics/
```
